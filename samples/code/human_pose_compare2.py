...
def draw_graph_fixed_result(
    frame_num_list, score_list, fixed_score_list, confidence_list, sync_data
):
    plt.plot(frame_num_list, score_list, label="Score")
...
def analysis_sync(
    ...
    ):  # 해당 sequence의 평균 dtw distance가 0.15 ~ 0.45 범위에 있을 경우
...
def sync_result_analysis(
    ...
    for i in range(sync_count):
        ...
        print(
            f"{sync_count}개의 싱크로율 문제 부분 중 {i + 1}번째 부분의 분석입니다. \n* 종료하시려면 ESC 버튼을 누르세요"
        )
        ...
        if sync_data[i][0][1] + sync_data[i][1] >= len(cam_frame_list) - 1:
            ...
            print(
                f"원본의 {i + 1}번째 구간({sync_data[i][0][0]} ~ {sync_data[i][0][1]} 프레임)의 평균 Score : {origin_avg_score}%"
            )
            print(
                f"개선본의 {i + 1}번째 구간({sync_data[i][0][0]} ~ {sync_data[i][0][1]} 프레임)의 평균 Score : {fix_avg_score}%"
            )
            print(
                f"개선 후 약 {round((fix_avg_score - origin_avg_score) / origin_avg_score, 3) * 100}% 만큼 Score가 향상되었습니다!"
            )
...
def main():
    ...
    if len(sync_data) >= 1:
        ...
        if slow_anal_check == "1":
            fixed_score = sync_result_analysis(
                ref_rendered_frames,
                ref_vectors,
                cam_frame_list,
                cam_vector_stack,
            ...
            draw_graph_fixed_result(
                frame_num_list, score_list, fixed_score, confidence_list, sync_data
            )
        else:
            pass
